<template>
    <div>
        <div v-if="hasData">
            ...
            <div class="itd_past" v-if="hasPastData">
                <h1 class="itd_past_tl">上月邀请明细</h1>
                <ul class="itd_past_wp">
                    <li v-for="item in pastData" class="m_inviteDetails_com">
                        <div>
                            <h3 class="m_inviteDetails_h3"><span class="m_inviteDetails_num1">{{item.nickName}}</span></h3>
                            <h3 class="m_inviteDetails_h3 m_inviteDetails_h3right">
                                <span class="m_inviteDetails_num2" v-bind:class="{ 'm_inviteDetails_bule': item.status == '2', 'm_inviteDetails_red': item.status == '3', 'm_inviteDetails_bl': !item.status == '1'  }">
        ...
        <div v-else class="m_inviteDetails_noct">
            <img src="./../assets/images/no-cnt.png" alt="图片加载失败">
            <h1>暂无更多数据哦~</h1>
        </div>
    </div>
</template>
...
<script>
    ...
    export default{
        ...
        computed:{
            ...mapState({
                hasData: state => state.invTk.hasData,
                hasNowData: state => state.invTk.hasNowData,
                hasPastData: state => state.invTk.hasPastData,
                nowData: state => state.invTk.nowData,
                pastData: state => state.invTk.pastData,
            })
        },
        created(){
            this.$store.dispatch('invTkDts');
        }
    }
</script>
